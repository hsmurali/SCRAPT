...
const size_t MAX_LENGTH = 3000;
std::vector<CharType> word(MAX_LENGTH + 1);
std::vector<PositionType> wordMinLength(MAX_LENGTH + 1);
std::vector<CostType> minCost(MAX_LENGTH + 1);
std::vector<PositionType> left(MAX_LENGTH + 1);
std::vector<PositionType> right(MAX_LENGTH + 1);
...
void initialize()
{
  ...
  left[0] = - static_cast<PositionType>(floor(MAX_LENGTH * (1 - similarity)));
  right[0] = std::numeric_limits<PositionType>::max() / 2;

  std::fill(word.begin(), word.end(), invalidChar);

  table(0, 0) = 0;
  backTable(0, 0) = ROOT;

  for (size_t j = 1; j < table.size2(); ++j) {
    table(0, j) = leftGapsAllowed ? 0 : table(0, j - 1) + cost(gapChar, invalidChar);
    backTable(0, j) = LEFT;
  }
...
AlignmentCostAndFunction findAlignmentFunction(const PositionType len1, const PositionType len2, const PositionType pos2)
{
  ...
  CostType minCost = table(len1, pos2);
...
PositionVector sortedStringsLengthsMinTree;
PositionVector sortedStringsLengthsMaxTree;
...
void makeCluster()
{
  ...
  if (! noKmerFilter) { 
    ...
    sortedStringsLengthsMinTree.resize(2 * sortedStrings.size());
    sortedStringsLengthsMaxTree.resize(2 * sortedStrings.size());

    buildMinTree(sortedStringsLengths.begin(), sortedStringsLengths.end(), sortedStringsLengthsMinTree.begin());
    buildMaxTree(sortedStringsLengths.begin(), sortedStringsLengths.end(), sortedStringsLengthsMaxTree.begin());

    word[1] = invalidChar;
    wordMinLength[1] = MAX_LENGTH + 1;
    searchResults.clear();

    if (numberOfUnmarkeds) {
      if (outputMultipleAlignment)
	search(0, numberOfUnmarkeds - 1, 0, 0);
      else
	search_without_backpointer(0, numberOfUnmarkeds - 1, 0, 0);
    }
    ...
    if (outputMultipleAlignment)
      search(0, sortedStrings.size() - 1, 0, 0);
    else
  ...
  { // Output cluster alignment.
    ...
    if (outputMultipleAlignment) { // Compute clusterCenterSequenceWithGaps.
      for (PositionType positionInQuery = 0; positionInQuery < querySequenceLength - 1; ++positionInQuery) {
	...
	if (!marked[lexicographicIndex]) {
	  ...
	  if (outputMultipleAlignment) {
	    ...
	    for (PositionType i = 0; i < static_cast<PositionType>(gapCounts.size() - 1); ++i) {
...
int main(int argc, char *argv[])
{
  ...
  { // Parse command line options.
    ...
    desc.add_options()
      ...
      ("no-k-mer-filter", po::bool_switch(&noKmerFilter)->default_value(false), "do not use k-mer filter")
...

  {
	...
	lexicographicIndexOfSortedStringsIndexes[i] = i;
      ...
      sortedStringsLengthsMinTree.resize(2 * numberOfInputSequences);
      sortedStringsLengthsMaxTree.resize(2 * numberOfInputSequences);

      buildMinTree(sequenceLengths.begin(), sequenceLengths.end(), sortedStringsLengthsMinTree.begin());
      buildMaxTree(sequenceLengths.begin(), sequenceLengths.end(), sortedStringsLengthsMaxTree.begin());
	...
	if ((numberOfInputSequences - numberOfMarked < remainingAtLastDbUpdate * 0.5) && (remainingAtLastDbUpdate > numberOfInputSequences * 0.05) && (numberOfInputSequences - numberOfMarked > 1)) {
	  if (! noKmerFilter) {
	    ...
	    buildMinTree(newSortedStringsLengths.begin(), newSortedStringsLengths.end(), sortedStringsLengthsMinTree.begin());
	    buildMaxTree(newSortedStringsLengths.begin(), newSortedStringsLengths.end(), sortedStringsLengthsMaxTree.begin());
